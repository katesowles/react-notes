...
#### React Elements
- React elements are just plain JS objects that *describe* a host instance. They're lightweight and have no host instance tied to them. As with host instances, React elements can form a tree by nesting elements within the child property of their "parent" element
...
- Since React's job is to make the host tree match the provided React element tree, there are two ways to go about reconciling differing commands:
  ...
  2. React decideds when to *update* a host intance and when to create a *new* one
...
ReactDOM.render(
  <parent>
    <conditional-child /> // has to update since type changed from `child-one` to `conditional-child`
    <child-one /> // has to create a new input since there was previously no second element
  </parent>,
  document.getElementById('root')
)
...
function Form({ showConditional }) {
  ...
  return (
    <parent>
      { conditionalChild }
      <child-one /> // regardless of if `showConditional` is true or false, `child-one` doesn't change position between renders
    </parent>
  );
...
- With dynamic lists, we can't be sure the order is ever the same, this is where object keys come in. A key tells React that it should consider an item to be *conceptually* the same even if it has a different *position* within a parent element between renders. This is why unique identifiers (like product IDs) are used.
...
### Component Purity
- In general, mutation isn't common within React, however, *local mutation* is absolutely fine. This means that within a Component function we can set a variable `items` and push items into this array variable within the component function before passing it to the `render` method, which is what "locks" it in.
...
ReactDOM.render( reactElement, document.getElementById('root') );
```
- What does React do when an element type is a function? It calls your component and asks what element *that* component wants to render within it.
```javascript
// This...
...
- This is a classic example of *inversion of control* (instead of with traditional programming, where the custom code that expresses the purpose of a program calls into a reusible library to take care of generic tasks, but with inversion of control, it is the framework that calls into the custom or task-specific code)
...
- Components are still functions but React augments them with features that are useful for UIs; these additional features are called *Hooks*
